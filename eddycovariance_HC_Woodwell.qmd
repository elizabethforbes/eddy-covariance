...
source("merge_management_data_timeseries_FINAL.r")

ec_daily_mngmnt <- create_management_vars_multicrop(
  ec_daily_avg,
  mngmnt) 
# %>% 
  ## add binary column of tillage: yes or no? yes = 0 (plotted on x axis), no = NA
  # mutate(tillage_event = case_when(
  # days_since_tillage == 0 ~ 0,
  # TRUE ~ NA))

chamber_daily_mngmnt <- chamber_daily_site %>% 
  mutate(date = as.Date(date, tz = "EST"))
chamber_daily_mngmnt <- create_management_vars_multicrop(
  chamber_daily_mngmnt,
  mngmnt) %>% 
  # add binary column for tillage as above
  mutate(tillage_event = case_when(
    days_since_tillage == 0 ~ 0, TRUE ~ NA
  )) %>% 
  mutate(year = as.numeric(year),
         month = as.numeric(month))

# write.csv(ec_daily_mngmnt, "ec_daily_mngmnt_esf.csv")
# write.csv(chamber_daily_mngmnt, "ch_daily_mngmnt_esf.csv")

```

I wrote this initial ec_daily_management df to .csv and made some manual changes to it, based on the management dataset and a line-by-line read to make sure the new continuous and categorical variables accurately reflect it. (see process notes document.) Most of these were done in the CSV file itself. Below, I am uploading the changed document. This involved doing some logic-based assignments of crop stage for the overwintering winter rye cover crop, and the one case of summer rye that overwintered on Kukon in 2020/2021.

```{r manually_edited_management_data}
...
<!-- ## Add PAR data to the hourly EC data: -->

<!-- ec_hrly_wpar <- ec_hrly_avg %>%  -->

<!--   left_join(pardat_hourly, -->

<!--             by = join_by(management, date, hour)) %>%  -->

<!--   # remove additional (possibly untrustworthy) soil and air temp, rh from PAR dataset -->

<!--   select(!c(soilt_C, airt_C, rh.y)) -->

<!-- # and to daily EC data: -->

<!-- ec_daily_wpar <- ec_daily_avg %>%  -->

<!--   left_join(pardat_dailymax, -->

<!--             by = join_by(management, date)) -->
...
ec_daily_mngmnt2 %>% 
  ggpairs(columns = c("VPD", "ndvi", "rh", 
                      "precip", "airt", "soilt_avg", "vwc_avg", 
                      "days_since_tillage"),
          progress = FALSE) # removes extensive printing

# which of these variables co-vary enough to reduce?
cor_vars <- ec_daily_mngmnt2 %>%
     select(VPD, ndvi, airt, soilt_avg, vwc_avg, rh, precip, days_since_tillage) %>%
     na.omit()

cor_matrix <- cor(cor_vars)
corrplot(cor_matrix, method = "number", type = "upper")

# covarying to a large degree: 
# air temp and soil temp. Keep soil temp, which is more directly affecting Reco and NEE.
# toss rh even though it and VPD don't covary (and in face, negatively co-vary by -0.36); it's likely not a hugely important driver and VPD is more mechanistically meaningful for gas exchange
...
AIC(g3, g3.2, g3.3)
      ...
      #   df       AIC
# g1	  43.08240	9253.917		
# g2	  69.64851	8976.697		
# g2.2	94.44709	8960.476		
...
newdata <- expand.grid(
  ...
  year = 2020,          # hold other variables constant at mean or typical values
...
AIC(r1, r1.2, r1.3)
# r1	77.71198	4843.723		
# r1.2	96.96036	4802.378		
# r1.3	103.57715	4774.922	

anova(r1.2, r1.3)
# 1    1679.8       1378                                     
# 2    1673.2       1347 6.5849   31.006 5.9219 1.555e-06 ***
...
Note: baseline comparison to conventional management and mature crop stage (estimated Reco = 3.55)
...
<!-- ## GAM: EC data, yearly cumulative -->

<!-- This analysis allows us to ask whether the systems differ in being a carbon sink or source and get closer to a total carbon budget. This analysis tells us whether the above differences in fluxes across management translate into whole-season differences in carbon balance. -->
