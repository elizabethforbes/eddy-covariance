...
make_pub_tables <- function(model,
  ...
  # NOTE: for gamm()-fitted models, summary(model$gam) can return zero-length
  # numeric(0) for some fields (observed for dev.expl) rather than NULL or NA.
  # is.null() alone does not catch a zero-length vector, and mixing a
  # zero-length value into tibble::tibble() silently collapses the *whole
  # row* to zero rows (via recycling) instead of erroring -- so we guard on
  # length() as well.
  .safe_scalar <- function(x, transform = identity) {
    if (is.null(x) || length(x) == 0) NA_real_ else transform(x)
  }

  stats <- tibble::tibble(
    model               = model_name,
    family              = if (length(sm$family$family) == 0) NA_character_ else sm$family$family,
    link                = if (length(sm$family$link) == 0) NA_character_ else sm$family$link,
    n                   = if (length(sm$n) == 0) NA_integer_ else sm$n,
    R2_adj              = .safe_scalar(sm$r.sq, \(x) round(x, decimals)),
    deviance_explained  = .safe_scalar(sm$dev.expl, \(x) round(x * 100, 1)),
    REML_or_GCV         = .safe_scalar(sm$sp.criterion, \(x) round(x, 2)),
    note                = r2_note
  )
